...
def threadSerial(infosend):
    ...
    for i in infosend:
        # print(i)
        writer+=str(i)
        # what is this value - Johnny
        # magnetPort.write(value)
        # what is this value bit-shifted? - Johnny
        # magnetPort.write(value >> 8)
        # magnetBuffer += (str(i) + ",")
        
    print(writer)
    magnetPort.write(writer)
...
def setup():
    global FLUID, GRID, WIDTH, D_RATE, VISCOSITY, TIME_SPACE, upperPort, lowerPort, img, sf, w, yvalues, magnetPort
    background(0)
...
def draw():
    ...
    DENS, DENS_PREV = FLUID.density_step(
        WIDTH, DENS, DENS_PREV, VEL_H, VEL_V, D_RATE, TIME_SPACE)
...
def count():
    ...
    for i in range(WIDTH):
        for j in range(WIDTH):
            magnitude = sqrt(VEL_V[FLUID.xy_coordinate(
                WIDTH, i, j)] ** 2 + VEL_H[FLUID.xy_coordinate(WIDTH, i, j)] ** 2)
            if magnitude > 0.2:
                if i < 10 and j < 10:
                    counter[0] += 1
                elif i >= 10 and j < 10:
...
def new_count():
    ...
    for i in range(WIDTH):
        for j in range(WIDTH):
            counter.append(
                ratio(DENS[FLUID.xy_coordinate(WIDTH, i + 1, j + 1)]))
...
def mouseReleased():
    ...
    VEL_H[FLUID.xy_coordinate(
        WIDTH, center_x + 1, center_y + 1)] = (mouseX - MOUSE_X) * magnitude // 2
    VEL_V[FLUID.xy_coordinate(
        WIDTH, center_x + 1, center_y + 1)] = (mouseY - MOUSE_Y) * magnitude // 2
...
def bubbly():
    DENS[FLUID.xy_coordinate(
        WIDTH, randint(0, 100) + 1, randint(0, 100) + 1)] = 256
    DENS[FLUID.xy_coordinate(
        WIDTH, randint(0, 100) + 1, randint(0, 100) + 1)] = 256
    DENS[FLUID.xy_coordinate(
        WIDTH, randint(0, 100) + 1, randint(0, 100) + 1)] = 256
...
def amoeba():
    ...
    if not ONCE:
        DENS[FLUID.xy_coordinate(
            WIDTH, randint(0, 3) + 1, randint(0, 3) + 1)] = randint(0, 256)
        DENS[FLUID.xy_coordinate(
            WIDTH, randint(0, 3) + 1, randint(0, 3) + 1)] = randint(0, 256)
        DENS[FLUID.xy_coordinate(
            WIDTH, randint(0, 3) + 1, randint(0, 3) + 1)] = randint(0, 256)
...
def reset_gridcells():
    ...
    for i in xrange(WIDTH):
        for j in xrange(WIDTH):
            GRID[i][j] = Cell(
                i * sf, j * sf, sf, sf, DENS[FLUID.xy_coordinate(WIDTH, i + 1, j + 1)])
